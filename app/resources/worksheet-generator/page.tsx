...
type Question = { stem: string; marks: number; paper: string; spec: string; answer: string; options?: string[]; correct?: number; lines: number };
...
export default function WorksheetGenerator() {
  ...
  const availablePapers = paperMap[level].filter(p => p.route === "Both" || p.route === route);
  ...
  return <main className="worksheet-site-page">
    ...
    {!preview ? <>
      ...
      <section className="worksheet-builder">
        ...
        <div className="worksheet-step"><Step n="02" title="Select the specification focus" text="Use the complete topic or choose individual learning outcomes."/><label>Topic<select value={topicIndex} onChange={e=>{setTopicIndex(Number(e.target.value));setSelected([])}}>{data.topics.map((t,i)=><option value={i} key={t.number}>{t.number}. {t.title}</option>)}</select></label><div className="worksheet-outcome-toolbar"><div><b>{data.topics[topicIndex]?.title}</b><small>{focus.length} eligible statements</small></div><button onClick={()=>setSelected(selected.length===focus.length?[]:focus.map(f=>f.id))}>{selected.length===focus.length?"Clear selection":"Select all"}</button></div><div className="worksheet-outcomes">{focus.map(item=><label className={selected.includes(item.id)?"active":""} key={`${item.id}-${item.tier}`}><input type="checkbox" checked={selected.includes(item.id)} onChange={()=>setSelected(ids=>ids.includes(item.id)?ids.filter(id=>id!==item.id):[...ids,item.id])}/><b>{item.id}</b><span>{item.text}<small>{item.subtopic} · {item.tier}</small></span></label>)}</div>{!selected.length&&<p className="worksheet-selection-note">No individual outcomes selected—the complete eligible topic will be used.</p>}</div>
        <div className="worksheet-step"><Step n="03" title="Set the worksheet details" text="The locked design template stays consistent across every level and topic."/><div className="worksheet-form-row two"><label>Worksheet title<input value={title} onChange={e=>setTitle(e.target.value)}/></label><label>Teacher or department<input value={teacher} onChange={e=>setTeacher(e.target.value)}/></label></div><label className="worksheet-switch-row">Include a separate teacher mark scheme <button className={includeScheme?"active":""} onClick={()=>setIncludeScheme(!includeScheme)}><i/></button></label></div>
