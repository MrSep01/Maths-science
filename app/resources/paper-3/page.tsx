...
import fullGuide from "./paper3-content.json";

const BOOKING_URL = "https://calendar.app.google/DxK139MN3MRwXxJ6A";
const STORAGE_KEY = "sep-paper-3-progress";
...
function sourceTableClass(rows: string[][]) {
  ...
  if (first.startsWith("🔬 PRACTICAL STRATEGY")) return "source-callout source-practical-strategy";
  if (first.startsWith("🏆 MARK SCHEME LANGUAGE")) return "source-callout source-mark-language";
  if (first.startsWith("📋 WORKED EXAMPLE")) return "source-worked-example";
...
export default function PaperThreeGuide() {
  ...
  return (
    <main className="paper-guide-page paper-three-page">
      ...
      <section className="paper-guide-hero" id="top">
        <div className="paper-guide-breadcrumb"><Link href="/resources">Resources</Link><span>›</span><b>Paper 3</b></div>
        <div className="paper-guide-hero-grid">
          <div>
            <p className="eyebrow"><span /> CIE A Level Chemistry · 9701</p>
            <h1>Paper 3<br /><em>Master Guide</em></h1>
            <p className="paper-guide-lead">The complete guide to advanced practical skills, qualitative analysis, quantitative techniques, data presentation, evaluation and laboratory exam strategy.</p>
          ...
          <div className="paper-one-cover original-guide-cover"><Image src="/paper-3-master-guide-cover.png" alt="Original Cambridge A Level Chemistry Paper 3 Master Revision Guide cover" width={1414} height={2000} priority /></div>
        </div>
        <div className="paper-stat-row"><div><b>40</b><span>Total marks</span></div><div><b>2 hours</b><span>Laboratory examination</span></div><div><b>11 + A</b><span>Guide sections</span></div><div><b>2018–2025</b><span>Analysis period</span></div></div>
      </section>

      <section className="source-front-matter" aria-label="Original Paper 3 guide front matter">
      ...
      <section className="guide-shell" id="guide-contents">
        <aside className="guide-sidebar">
          ...
          <nav aria-label="Paper 3 table of contents"><p>Table of contents</p>{fullGuide.sections.map((section) => <a href={`#${section.id}`} key={section.id} className={completed.includes(section.id) ? "is-complete" : ""}><span>{completed.includes(section.id) ? "✓" : sectionNumber(section.id)}</span>{displaySectionTitle(section.title)}</a>)}</nav>
        </aside>

        <div className="guide-main">
          <div className="guide-introduction"><p className="eyebrow"><span /> Complete original guide</p><h2>Measure carefully.<br /><em>Record scientifically.</em></h2><p>Every source paragraph and table is presented below in its original order. Mark sections complete as you work; progress is saved on this device.</p></div>
      ...
      <section className="paper-support-cta" id="study-support"><div><p className="eyebrow light"><span /> Personal Paper 3 support</p><h2>Develop the method.<br /><em>Protect every practical mark.</em></h2></div><div><p>Use recent practical work or a completed paper to identify weaknesses in observations, tables, graphs, calculations, conclusions and evaluation.</p><a className="button paper-support-button" href={BOOKING_URL} target="_blank" rel="noreferrer">Book a free consultation <span>↗</span></a></div></section>
