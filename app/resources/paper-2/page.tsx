...
import fullGuide from "./paper2-content.json";

const BOOKING_URL = "https://calendar.app.google/DxK139MN3MRwXxJ6A";
const STORAGE_KEY = "sep-paper-2-progress";
...
function sourceTableClass(rows: string[][]) {
  ...
  if (first.startsWith("✍️ STRUCTURED Q STRATEGY")) return "source-callout source-strategy";
  if (first.startsWith("📋 COMMAND WORD GUIDE")) return "source-callout source-command-guide";
  if (first.startsWith("🏆 MARK SCHEME LANGUAGE")) return "source-callout source-mark-language";
  if (first.startsWith("📝 WORKED ANSWER EXAMPLE")) return "source-worked-example";
...
export default function PaperTwoGuide() {
  ...
  return (
    <main className="paper-guide-page paper-two-page">
      ...
      <section className="paper-guide-hero" id="top">
        <div className="paper-guide-breadcrumb"><Link href="/resources">Resources</Link><span>›</span><b>Paper 2</b></div>
        <div className="paper-guide-hero-grid">
          <div>
            <p className="eyebrow"><span /> CIE A Level Chemistry · 9701</p>
            <h1>Paper 2<br /><em>Master Guide</em></h1>
            <p className="paper-guide-lead">The complete guide to AS Level structured questions, command words, calculations, mark-scheme language, worked answers and exam-day strategy.</p>
          ...
          <div className="paper-one-cover original-guide-cover"><Image src="/paper-2-master-guide-cover.png" alt="Original Cambridge A Level Chemistry Paper 2 Master Revision Guide cover" width={1414} height={2000} priority /></div>
        </div>
        <div className="paper-stat-row"><div><b>60</b><span>Total marks</span></div><div><b>1h 15m</b><span>Examination time</span></div><div><b>10 + A</b><span>Guide sections</span></div><div><b>2018–2025</b><span>Analysis period</span></div></div>
      </section>

      <section className="source-front-matter" aria-label="Original Paper 2 guide front matter">
      ...
      <section className="guide-shell" id="guide-contents">
        <aside className="guide-sidebar">
          ...
          <nav aria-label="Paper 2 table of contents"><p>Table of contents</p>{fullGuide.sections.map((section) => <a href={`#${section.id}`} key={section.id} className={completed.includes(section.id) ? "is-complete" : ""}><span>{completed.includes(section.id) ? "✓" : sectionNumber(section.id)}</span>{displaySectionTitle(section.title)}</a>)}</nav>
        </aside>

        <div className="guide-main">
          <div className="guide-introduction"><p className="eyebrow"><span /> Complete original guide</p><h2>Read, decode, plan<br /><em>and answer.</em></h2><p>Every source paragraph and table is presented below in its original order. Mark sections complete as you work; progress is saved on this device.</p></div>
      ...
      <section className="paper-support-cta" id="study-support"><div><p className="eyebrow light"><span /> Personal Paper 2 support</p><h2>Understand the chemistry.<br /><em>Write for the marks.</em></h2></div><div><p>Use a recent structured paper to identify where knowledge, calculation setup, command words or mark-scheme language are limiting the student’s score.</p><a className="button paper-support-button" href={BOOKING_URL} target="_blank" rel="noreferrer">Book a free consultation <span>↗</span></a></div></section>
