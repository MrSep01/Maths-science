...
import fullGuide from "./paper5-content.json";

const BOOKING_URL = "https://calendar.app.google/DxK139MN3MRwXxJ6A";
const STORAGE_KEY = "sep-paper-5-progress";
...
export default function PaperFiveGuide() {
  ...
  return (
    <main className="paper-guide-page paper-five-page">
      ...
      <section className="paper-guide-hero" id="top">
        <div className="paper-guide-breadcrumb"><Link href="/resources">Resources</Link><span>›</span><b>Paper 5</b></div>
        <div className="paper-guide-hero-grid">
          <div>
            <p className="eyebrow"><span /> CIE A Level Chemistry · 9701</p>
            <h1>Paper 5<br /><em>Master Guide</em></h1>
            <p className="paper-guide-lead">The complete guide to experimental planning, variables, method design, graphing, data analysis, scientific conclusions and evaluation.</p>
          ...
          <div className="paper-one-cover original-guide-cover"><Image src="/paper-5-master-guide-cover.png" alt="Original Cambridge A Level Chemistry Paper 5 Master Revision Guide cover" width={1414} height={2000} priority /></div>
        </div>
        <div className="paper-stat-row"><div><b>30</b><span>Total marks</span></div><div><b>1h 15m</b><span>Examination time</span></div><div><b>10 + A</b><span>Guide sections</span></div><div><b>2018–2025</b><span>Analysis period</span></div></div>
      </section>

      <section className="source-front-matter" aria-label="Original Paper 5 guide front matter">
      ...
      <section className="guide-shell" id="guide-contents">
        <aside className="guide-sidebar">
          ...
          <nav aria-label="Paper 5 table of contents"><p>Table of contents</p>{fullGuide.sections.map((section) => <a href={`#${section.id}`} key={section.id} className={completed.includes(section.id) ? "is-complete" : ""}><span>{completed.includes(section.id) ? "✓" : sectionNumber(section.id)}</span>{displaySectionTitle(section.title)}</a>)}</nav>
        </aside>

        <div className="guide-main">
          <div className="guide-introduction"><p className="eyebrow"><span /> Complete original guide</p><h2>Plan methodically.<br /><em>Evaluate scientifically.</em></h2><p>Every source paragraph and table is presented below in its original order. Mark sections complete as you work; progress is saved on this device.</p></div>
      ...
      <section className="paper-support-cta" id="study-support"><div><p className="eyebrow light"><span /> Personal Paper 5 support</p><h2>Turn an investigation<br /><em>into a scoring answer.</em></h2></div><div><p>Use a recent Paper 5 response to identify weaknesses in variables, method detail, graph interpretation, scientific conclusions, error analysis and realistic improvements.</p><a className="button paper-support-button" href={BOOKING_URL} target="_blank" rel="noreferrer">Book a free consultation <span>↗</span></a></div></section>
